...
data class Movie(
    ...
    @SerializedName(Genres.GenresEntry.GENRES)
    val genres: List<Genres>,
    @SerializedName(Produce.ProduceEntry.PRODUCES)
    val produce: List<Produce>,
    @SerializedName(Cast.CastEntry.CREDITS)
    val casts: CastCredits,
    @SerializedName(MovieTrailer.MovieTrailerEntry.MOVIE_TRAILER_VIDEO)
    val trailers: MovieTrailerVideos
) {
    data class CastCredits(
        @SerializedName(Cast.CastEntry.CAST)
        val data: List<Cast>)
    data class
    MovieTrailerVideos(
        @SerializedName(MovieTrailer.MovieTrailerEntry.MOVIE_TRAILER_RESULTS)
